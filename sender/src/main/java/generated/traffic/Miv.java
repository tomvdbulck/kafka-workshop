...
/**
 * Meten-in-Vlaanderen (MIV) Loop-based traffic data (1-minute data). - http://opendata.vlaanderen.be/dataset/minuutwaarden-verkeersmetingen-vlaanderen - The data is provided by the inductive detection loops, mainly on highways in Flanders. - The data controllers are Agentschap Wegen en Verkeer (Roads and Traffic Agency, http://www.wegenenverkeer.be) and Vlaams Verkeerscentrum (the Flemish Traffic Centre, http://www.verkeerscentrum.be). - The traffic data contained are p.e. the number of vehicles and the average speeds, divided in 5 vehicle classes, aggregated per minute. - The data is updated every minute. - The configuration data (location, ...) of the measurement points is available at http://miv.opendata.belfla.be/miv/configuratie/xml - The traffic data is available at http://miv.opendata.belfla.be/miv/verkeersdata - The xsd-schemes document all data fields and are available at: http://miv.opendata.belfla.be/miv-config.xsd http://miv.opendata.belfla.be/miv-verkeersdata.xsd - Feedback and suggestions for improvement: klantendienst-awv@wegenenverkeer.be, reference: OPENDATA.MIV - Availability and liability: see art. 8 of the license. Unavailability of this dataset can be notified to vtc.meldingen@vlaanderen.be (Dutch only), reference: OPENDATA.MIV - License: Modellicentie Gratis Hergebruik - v1.0 
 * 
